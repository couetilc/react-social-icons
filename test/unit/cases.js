...
export const cases = SocialIcon => describe('<SocialIcon />', () => {
  ...
  it('includes child elements within anchor', ({ expect }) => {
    render(<SocialIcon url={pinterest_url}><div>child</div></SocialIcon>)
  ...
  it('adds rel attribute to anchor', ({ expect }) => {
    ...
    expect(screen.getByTestId(true)).toHaveAttribute('rel', 'noopener noreferrer')
  })
  ...
  it('overrides aria label on anchor with prop `aria-label`', ({ expect }) => {
    render(<SocialIcon url={pinterest_url} aria-label='override' />)
    expect(link()).toHaveAttribute('aria-label', 'override')
  })

  it('overrides aria label on anchor with correct order', ({ expect }) => {
    render(<SocialIcon url={pinterest_url} label='override' aria-label='aria' />)
    expect(link()).toHaveAttribute('aria-label', 'override')
  })
  ...
  it('override fallback icon for social svg with an object', ({ expect }) => {
    render(<SocialIcon url='https://example.com' fallback={{
      icon: 'test-icon',
      mask: 'test-mask',
  ...
  it('[deprecated] override default icon for social svg', ({ expect }) => {
    render(<SocialIcon url='https://example.com' defaultSVG={{
      icon: 'test-icon',
      mask: 'test-mask',
      color: 'rgb(254,254,254)',
    }} />)
  ...
  it('allows for separate default icon overrides for social svg instances', ({ expect }) => {
    ...
    const { rerender } = render(
      <SocialIcon url='https://example.com' fallback={{
  ...
  it('href prop is undefined when neither url nor href props are present', ({ expect }) => {
  ...
  it('"as" prop will override <SocialIcon> element type', ({ expect }) => {
    ...
    expect(screen.getByLabelText('sharethis')).toHaveAttribute('class', 'social-icon')
  })

  it('renders children', ({ expect }) => {
    render(<SocialIcon data-testid><div id="test" /></SocialIcon>)
